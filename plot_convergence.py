...
sparse_scipy_cpu_time = sparse_scipy_data['e_infty']
sparse_scipy_n = sparse_scipy_data['n']
sparse_custom_cpu_time = sparse_custom_data['e_infty']
sparse_custom_n = sparse_custom_data['n']
dense_custom_cpu_time = dense_custom_data['e_infty']
dense_custom_n = dense_custom_data['n']

# Discretisation Convergence plot
theoretical_scipy = sparse_scipy_cpu_time[-1] * (sparse_scipy_n / sparse_scipy_n[-1])**(-2)
theoretical_custom = sparse_custom_cpu_time[-1] * (sparse_custom_n / sparse_custom_n[-1])**(-2)
theoretical_dense = dense_custom_cpu_time[-1] * (dense_custom_n / dense_custom_n[-1])**(-2)

plt.figure(figsize=(8, 5))
plt.loglog(sparse_scipy_n, theoretical_scipy, label='Theoretical O($h^{2}$)', linestyle='--', color=viridis(0.7), linewidth=2.2)
