...
sparse_scipy_cpu_time = sparse_scipy_data['direct_solver_cpu_times']
sparse_scipy_n = sparse_scipy_data['n']
sparse_custom_cpu_time = sparse_custom_data['direct_solver_cpu_times']
sparse_custom_n = sparse_custom_data['n']
dense_custom_cpu_time = dense_custom_data['direct_solver_cpu_times']
dense_custom_n = dense_custom_data['n']

# Discretisation Convergence plot
theoretical_scipy = sparse_scipy_cpu_time[-1] * (sparse_scipy_n / sparse_scipy_n[-1])**(3)
theoretical_custom = sparse_custom_cpu_time[-1] * (sparse_custom_n / sparse_custom_n[-1])**(3)
theoretical_dense = dense_custom_cpu_time[-1] * (dense_custom_n / dense_custom_n[-1])**(3)

plt.figure(figsize=(8, 5))
plt.loglog(sparse_scipy_n, theoretical_scipy, label='Theoretical O($n^3$)', linestyle='--', color=viridis(0.7), linewidth=2.2)
...
# Discretisation Convergence plot
theoretical_scipy = sparse_scipy_cpu_time[-1] * (sparse_scipy_n / sparse_scipy_n[-1])**(5)
theoretical_custom = sparse_custom_cpu_time[-1] * (sparse_custom_n / sparse_custom_n[-1])**(5)
theoretical_dense = dense_custom_cpu_time[-1] * (dense_custom_n / dense_custom_n[-1])**(5)

plt.figure(figsize=(8, 5))
plt.loglog(sparse_scipy_n, theoretical_scipy, label='Theoretical O($n^5$)', linestyle='--', color=viridis(0.7), linewidth=2.2)
plt.loglog(sparse_custom_n, theoretical_custom, linestyle='--', color=viridis(0.7), linewidth=2.2)
plt.loglog(dense_custom_n, theoretical_dense, linestyle='--', color=viridis(0.7), linewidth=2.2)
